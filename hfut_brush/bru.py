...
class Brush:
    ...
    def __init__(self, ID=None, Pwd=None, **kwargs):
        self.ID = ID or input("请输入学号\n")
        self.Pwd = Pwd or getpass.getpass("请输入密码\n")
    ...
    def try_login(self):
        ...
        tried_times = 1
        while last_times:
            message, if_login_success = self.login()
            if if_login_success:
                return True
            tried_times += 1
            last_times -= 1
        else:
            message = "Login failed after tried the max_times"
            print(message)
            return False
    ...
    def login(self):
        ...
        if res.text.find("验证码错误") != -1:
            ...
            return message, if_login_success
            # time.sleep(0.01)
    ...
    def start(self):
        finished = False
        if_login_success = self.try_login()
        while not finished and if_login_success:
            self.main()
